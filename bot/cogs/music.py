...
class Player(wavelink.Player):
    ...
    async def add_tracks(self, ctx, tracks):
        ...
        if isinstance(tracks, wavelink.TrackPlaylist):
            ...
            embed=discord.Embed()
            embed.add_field(name="Added to the queue", value=tracks[0].title, inline=False)
            await ctx.send(embed=embed)
        else:
            if (track := await self.choose_track(ctx, tracks)) is not None:
                self.queue.add(track)
                embed=discord.Embed()
                embed.add_field(name="Added to the queue", value=track.title, inline=False)
                await ctx.send(embed=embed)
...
class Music(commands.Cog, wavelink.WavelinkMixin):
    ...
    async def cog_check(self, ctx):
        if isinstance(ctx.channel, discord.DMChannel):
            ...
            embed.description = ("Music commands are not available in DMS!")
            await ctx.send(embed=embed)
            return False
    ...
    async def connect_command_error(self, ctx, exc):
        if isinstance(exc, AlreadyConnectedToChannel):
            embed=discord.Embed(color=0xff0000)
            embed.description = ("Already connected to the channel!")
            await ctx.send(embed=embed)
    ...
    async def play_command(self, ctx, *, query: t.Optional[str]):
        ...
        if query is None:
            ...
            embed=discord.Embed()
            embed.description = ("Music resumed")
            await ctx.send(embed=embed)
    ...
    async def play_command_error(self, ctx, exc):
        if isinstance(exc, QueueIsEmpty):
            ...
            embed = discord.Embed(color=0xff0000)
            embed.description = ("No tracks could be found.")
            await ctx.send(embed=embed)
    ...
    async def pause_command(self, ctx):
        ...
        embed=discord.Embed()
        embed.description = ('Music paused.')
        await ctx.send(embed=embed)
    ...
    async def pause_command_error(self, ctx, exc):
        if isinstance(exc, PlayerIsAlreadyPaused):
            embed=discord.Embed()
            embed.description = ("Music is already paused")
            await ctx.send(embed=embed)
    ...
    async def stop_command(self, ctx):
        ...
        await player.stop()
        embed=discord.Embed()
        embed.description = ("Music stopped.")
        await ctx.send(embed=embed)
    ...
    async def next_command(self, ctx):
        ...
        await player.stop()
        embed=discord.Embed()
        embed.description = ("Now playing the next track in the queue.")
        await ctx.send(embed=embed)
    ...
    async def invite_link(self, ctx):
        embed = discord.Embed()
        embed.add_field(name= "bot invite link", value="Invite the bot to your server by clicking [here](https://discord.com/api/oauth2/authorize?client_id=843157115564589086&permissions=8&scope=bot)")
        await ctx.send(embed=embed)
...
# --Normal commands--
    ...
    async def shutdown(self, ctx):
        if ctx.author.id == 384759989644034049:
            await ctx.send('Shutting the bot down!')
            await ctx.message.delete()
            await ctx.bot.logout()
        else:
            await ctx.send("You are not the bot developer!")
    ...
    async def serverinfo(self, ctx, member: discord.member = None):
        ...
        embed2.add_field(name='Highest role', value=member.top_role.mention, inline=False)
    ...
    @commands.command(name="binfo", aliases=["botinfo"])
    async def binfo(self, ctx, member: discord.Member = None):
        if not member:
            member = ctx.message.author
        embed=discord.Embed(title="Bot info")
        embed.set_author(name="LMusic")
        embed.add_field(name="Join date", value="Sat, 15 May 2021", inline=True)
        embed.add_field(name="ID", value="843157115564589086", inline=True)
        embed.add_field(name="TOKEN", value="||haha you thought, the ID is above tho||", inline=False)
        embed.add_field(name="Bot developer", value="pl0x#0001", inline=True)
        embed.set_footer(text=f"Requested by {ctx.author}")
        await ctx.send(embed=embed)
